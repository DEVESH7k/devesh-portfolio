...
const featuredDays = [25, 26, 27, 28, 29, 30];

function Writing() {
  const [showAll, setShowAll] = useState(false);

  const featuredPosts = blogPosts.filter((p) => featuredDays.includes(p.day));
  const allPostsSorted = [...blogPosts].sort((a, b) => a.day - b.day);
  const displayPosts = showAll ? allPostsSorted : featuredPosts;
    ...
    <section id="writing" className="section-padding relative overflow-hidden">
      ...
      >
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          <p className="text-[11px] font-mono text-[#b4aec8] uppercase tracking-[0.18em]">
            {showAll ? "All 30 Posts" : "Featured Posts"}
          </p>
          <button
            onClick={() => setShowAll((v) => !v)}
            className="flex items-center gap-2 px-4 py-2 rounded-full border border-accent/30 text-accent text-[11px] font-mono uppercase tracking-wider hover:bg-accent/10 transition-all duration-300"
          >
            {showAll ? (
              <>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`grid gap-5 ${showAll ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"}`}
          >
            {displayPosts.map((post) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="bg-bg1 border border-white/[0.06] border-t-0 rounded-b-2xl p-6 h-full flex flex-col">
                    ...
                    <h3 className={`font-outfit font-semibold text-white leading-snug group-hover:text-accent transition-colors duration-300 ${showAll ? "text-[14px]" : "text-[17px]"}`}>
                      {post.title}
                    </h3>
                    ...
                    <div className="mt-4 flex items-center gap-2 text-[12px] font-mono text-accent opacity-0 group-hover:opacity-100 transition-opacity duration-300" aria-hidden="true">
                      Read on LinkedIn
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
